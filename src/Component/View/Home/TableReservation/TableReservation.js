      ...
      <div className="reservation-container">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="reservation-form">
          <div className="form-row">
            ...
            <div className="form-group-reservation">
              <label>{customTranslate("Phone")}:</label>
              <input
                type="tel"
          ...
          <div className="form-row">
            <div className="form-group-reservation">
              <label>{customTranslate("Date")}:</label>
              <input
                type="date"
            ...
            <div className="form-group-reservation">
              <label>{customTranslate("Time")}:</label>
              <select {...register("time", { required: true })}>
                <option value="">{customTranslate("Select time")}</option>
            ...
            <div className="form-group-reservation">
              <label>{customTranslate("Person")}:</label>
              <select
                {...register("guests", { required: true })}
                onChange={onGuestChange}
              >
                <option value="">{customTranslate("Many persons")}?</option>
                {Array.from({ length: 8 }, (_, index) => {
                  const numberOfGuests = index + 1;
                  return (
                    <option key={numberOfGuests} value={numberOfGuests}>
                      {numberOfGuests}{" "}
                     {customTranslate(`${numberOfGuests > 1 ? "Persons" : "Person"}`)}
                    </option>
                  );
                })}
              </select>
            ...
            <div className="form-group-reservation">
              <label>{customTranslate("Table")}:</label>
              <select {...register("tableId", { required: true })}>
                <option value="">{customTranslate("Select a table")}</option>
                {tables.length === 0 ? (
                  <option disabled>{customTranslate("No available tables")}</option> // Hiển thị nếu không có bàn nào
                ) : (
                  tables.map((table) => (
                    <option key={table.tableId} value={table.tableId}>
                      {table.tableName} - {customTranslate("Capacity")}: {table.capacity}
                    </option>
                  ))
                )}
              </select>
          ...
          <div className="submit-button-wrapper">
            <button type="submit" className="submit-button">
              {customTranslate("Book a table")}
            </button>
          </div>
          {message && <p className="success-message">{customTranslate(`${message}`)}</p>}
        </form>
      </div>
    </div>
  );
